...
def classifyPerson():
    ...
    ffMiles = float(input("time spent flying"))
    iceCream = float(input("tons of icecream"))
    datingDataMat,datingLabels = file2matrix('datingTestSet2.txt')
    normMat,ranges,minVals = autoNorm(datingDataMat)
...
def handwritingClassTest():
    ...
    for i in range(mTest):
        ...
        print('result : %d,real number : %d'%(classifierResult,classNumStr))
        if(classifierResult != classNumStr):
            errorCount += 1.0
    print('number of error : %d'%errorCount)
